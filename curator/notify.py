...
def build_slack_text(added_by_playlist: dict[str, list[ScoredVideo]]) -> str:
    """Slack 메시지 본문(mrkdwn)을 만든다.

    Slack 은 `<url|텍스트>` 형식의 링크와 `*굵게*` 를 지원하므로 제목을 링크로 건다.
    """
    today = datetime.now(_KST).strftime("%Y-%m-%d")
    total = sum(len(v) for v in added_by_playlist.values())

    lines = [f"*[YouTube Curator] {today} 새 영상 {total}개 추가됨*", ""]
    for name, videos in added_by_playlist.items():
        if not videos:
            continue
        lines.append(f"▶ *{name}* ({len(videos)}개)")
        for v in videos:
            lines.append(
                f"  • [{v.score}] <{v.url}|{v.title}>"
                f"  ({_fmt_duration(v.duration_seconds)}) — {v.channel}"
            )
        lines.append("")
...
def send_slack_summary(added_by_playlist: dict[str, list[ScoredVideo]]) -> bool:
    """추가된 영상 요약을 Slack Incoming Webhook 으로 발송한다.

    추가된 영상이 없거나 SLACK_WEBHOOK_URL 이 설정되지 않았으면 발송하지 않고
    False 를 반환한다. 발송에 성공하면 True.
    """
    total = sum(len(v) for v in added_by_playlist.values())
    if total == 0:
        print("[notify] 추가된 영상이 없어 Slack 알림을 보내지 않습니다.")
        return False

    webhook_url = os.environ.get("SLACK_WEBHOOK_URL")
    if not webhook_url:
        print("[notify] SLACK_WEBHOOK_URL 미설정 — Slack 알림을 건너뜁니다.")
        return False

    payload = json.dumps({"text": build_slack_text(added_by_playlist)}).encode("utf-8")
    ...
    try:
        ...
        print(f"[notify] Slack 발송 실패 — {exc}")
        return False

    print(f"[notify] Slack 발송 완료 — {total}개 영상 요약을 보냈습니다.")
    return True
